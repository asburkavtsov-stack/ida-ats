...
function StageBadge({ candidate }) {
  ...
  return (
    <span style={{ fontSize:'0.66rem', fontFamily:'DM Mono', padding:'3px 8px', borderRadius:'4px', background:`rgba(${r},${g},${b},0.12)`, color }}>
      {name}
    </span>
  );
}
...
const CHECKBOX = (checked, indeterminate = false) => (
  <div style={{
    ...
    background: checked ? 'var(--accent)' : indeterminate ? 'rgba(159,18,57,0.15)' : 'var(--surface)',
  ...
  return (
    <div style={{ padding: isMobile ? '8px' : '0' }}>
      ...
      <div style={{ display:'flex', gap:'8px', marginBottom:'16px', alignItems:'center', flexWrap:'wrap' }}>
        <button onClick={() => setMineFilter(!mineFilter)} style={{
        ...
        {activeStatusFilters.map(f => (
          <button key={f.key} onClick={() => setFilter(f.key)} style={{
            ...
            background: filter === f.key ? (f.color ? f.color + '20' : 'var(--accent)') : 'var(--surface)',
        ...
        {availableTags.length > 0 && (
          <>
            ...
            {availableTags.map(tag => (
              <button key={tag.id} onClick={() => setTagFilter(prev =>
                prev.includes(tag.id) ? prev.filter(id => id !== tag.id) : [...prev, tag.id]
              )} style={{
                padding:'4px 10px', borderRadius:'20px', fontSize:'0.72rem', fontWeight:500, cursor:'pointer',
      ...
      <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:'12px', gap:'8px' }}>
        <div style={{ display:'flex', alignItems:'center', gap:'10px' }}>
          ...
          <button onClick={() => { setBulkMode(v => !v); setSelectedIds([]); }} style={{
            padding:'5px 12px', borderRadius:'7px', fontSize:'0.75rem',
        ...
        <button onClick={handleExportCSV} disabled={exporting} style={{
          ...
          fontSize:'0.78rem', fontFamily:'DM Mono', opacity: exporting ? 0.6 : 1,
        }}>
        ...
        {isMobile ? (
          <div>
            {loading ? (
              ...
              return (
                <div key={c.id || i}
                  onClick={() => bulkMode ? setSelectedIds(prev => prev.includes(c.id) ? prev.filter(x => x !== c.id) : [...prev, c.id]) : setSelectedCandidateId(c.id)}
                  style={{ padding:'14px 16px', borderBottom:'1px solid var(--border)', cursor:'pointer', transition:'background 0.1s', background: isSelected ? 'rgba(159,18,57,0.05)' : 'transparent', display:'flex', gap:'10px', alignItems:'flex-start' }}
                  onMouseEnter={e => { if (!isSelected) e.currentTarget.style.background = 'var(--bg)'; }}
                  onMouseLeave={e => { e.currentTarget.style.background = isSelected ? 'rgba(159,18,57,0.05)' : 'transparent'; }}
                >
                  {bulkMode && <div onClick={e => toggleSelect(e, c.id)} style={{ marginTop:'3px' }}>{CHECKBOX(isSelected)}</div>}
                  <div style={{ flex:1, minWidth:0 }}>
                    <div style={{ display:'flex', justifyContent:'space-between', alignItems:'flex-start', marginBottom:'8px' }}>
                      ...
                      <div style={{ display:'flex', gap:'6px', flexShrink:0 }}>
                        ...
                        <span style={{ fontSize:'0.66rem', fontFamily:'DM Mono', padding:'3px 8px', borderRadius:'4px', background:getSourceBg(c.source), color:getSourceText(c.source) }}>{getSourceLabel(c.source)}</span>
                      </div>
                    </div>
                    ...
                    <div style={{ display:'flex', alignItems:'center', gap:'8px', marginTop:'4px' }}>
                      {c.assigned_to && (
                        <div style={{ width:'20px', height:'20px', borderRadius:'50%', background:getHrAvatarColor(c.assigned_to), display:'flex', alignItems:'center', justifyContent:'center', fontSize:'0.55rem', fontWeight:700, color:'#fff' }} title={c.assigned_to_name}>
                          {(c.assigned_to_name?.[0] || '?').toUpperCase()}
                        </div>
                      )}
          ...
          <table style={{ width:'100%', borderCollapse:'collapse' }}>
              ...
              <tr style={{ background:'var(--bg)' }}>
                {bulkMode && (
                  <th style={{ padding:'11px 16px', width:'40px', borderBottom:'1px solid var(--border)' }}>
                    <div onClick={toggleAll}>{CHECKBOX(allSelected, someSelected)}</div>
                  </th>
                )}
            ...
            <tbody>
              {loading ? (
                ...
                return (
                  <tr key={c.id || i}
                    style={{ borderBottom:'1px solid var(--border)', cursor:'pointer', transition:'background 0.1s', background: isSelected ? 'rgba(159,18,57,0.05)' : 'transparent' }}
                    onClick={() => bulkMode ? setSelectedIds(prev => prev.includes(c.id) ? prev.filter(x => x !== c.id) : [...prev, c.id]) : setSelectedCandidateId(c.id)}
                    onMouseEnter={e => { if (!isSelected) e.currentTarget.style.background = 'var(--bg)'; }}
                    onMouseLeave={e => { e.currentTarget.style.background = isSelected ? 'rgba(159,18,57,0.05)' : 'transparent'; }}
                  >
                    {bulkMode && (
                      <td style={{ padding:'13px 16px' }} onClick={e => toggleSelect(e, c.id)}>
                        {CHECKBOX(isSelected)}
                      </td>
                    )}
                    ...
                    <td style={{ padding:'13px 16px' }}><StageBadge candidate={c} /></td>
                    <td style={{ padding:'13px 16px' }}>
                    ...
                    <td style={{ padding:'13px 16px' }}>
                      <div style={{ display:'flex', alignItems:'center', gap:'8px' }}>
                        {c.assigned_to && (
                          <div style={{ width:'22px', height:'22px', borderRadius:'50%', background:getHrAvatarColor(c.assigned_to), display:'flex', alignItems:'center', justifyContent:'center', fontSize:'0.6rem', fontWeight:700, color:'#fff', flexShrink:0 }} title={c.assigned_to_name}>
                            {(c.assigned_to_name?.[0] || '?').toUpperCase()}
                          </div>
                        )}
      ...
      <BulkActionBar
        ...
        onDone={() => { fetchCandidates(currentPage); setSelectedIds([]); }}
      />
      ...
      {selectedCandidateId && (
        <CandidateCardModal
          ...
          onDelete={(id) => { setCandidates(prev => prev.filter(c => c.id !== id)); setSelectedCandidateId(null); setTotalCount(prev => prev - 1); }}
        />
      )}
    </div>
  );
