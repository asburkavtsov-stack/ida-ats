// Kanban.js — динамічні колонки + фільтри + редактор стейджів + bulk actions
...
function StageEditor({ stage, onSave, onDelete, onClose }) {
  ...
  return (
    <div style={{ position:'absolute', top:'calc(100% + 6px)', left:0, zIndex:300, background:'var(--surface)', border:'1px solid var(--border)', borderRadius:'12px', padding:'16px', width:'240px', boxShadow:'0 8px 32px rgba(0,0,0,0.18)' }}>
      ...
      <input value={name} onChange={e => setName(e.target.value)} placeholder="Назва етапу" autoFocus style={{ width:'100%', padding:'8px 10px', border:'1px solid var(--border)', borderRadius:'7px', fontSize:'0.85rem', fontFamily:'DM Sans', background:'var(--bg)', marginBottom:'10px', outline:'none' }} />
      ...
      <div style={{ display:'flex', gap:'6px' }}>
        ...
        {!isNew && onDelete && (
          <button onClick={() => { onDelete(stage.id); onClose(); }} style={{ padding:'7px 10px', borderRadius:'7px', border:'none', background:'#fee2e2', color:'#dc2626', cursor:'pointer', fontSize:'0.78rem' }}>🗑</button>
        )}
        <button onClick={() => name.trim() && onSave({...stage, name:name.trim(), color, is_terminal:isTerminal})} disabled={!name.trim()} style={{ flex:1, padding:'7px', borderRadius:'7px', border:'none', background: name.trim() ? 'var(--accent)' : 'var(--border)', color: name.trim() ? '#fff' : 'var(--muted)', cursor: name.trim() ? 'pointer' : 'not-allowed', fontSize:'0.78rem', fontWeight:600 }}>Зберегти</button>
      </div>
    </div>
  );
}
...
// ─── Картка кандидата ─────────────────────────────────────────────────────────
function CandidateCard({ candidate, onOpen, bulkMode, selected, onToggle }) {
  return (
    <div
      draggable={!bulkMode}
      onDragStart={e => {
...
// ─── Колонка ──────────────────────────────────────────────────────────────────
function KanbanColumn({ stage, candidates, onDrop, onOpen, onEdit, onDelete, isFirst, isLast, onMoveLeft, onMoveRight, isMobile, bulkMode, selectedIds, onToggle }) {
  const [dragOver,   setDragOver]   = useState(false);
  const [showEditor, setShowEditor] = useState(false);
  const colSelected = candidates.filter(c => selectedIds.includes(c.id)).length;

  return (
    <div style={{ minWidth: isMobile ? '82vw' : '240px', maxWidth: isMobile ? '82vw' : '280px', flex:'0 0 auto', display:'flex', flexDirection:'column', background: dragOver ? hex2rgba(stage.color, 0.06) : 'var(--bg)', border: dragOver ? `2px dashed ${stage.color}` : '2px solid transparent', borderRadius:'12px', transition:'all 0.15s', padding:'6px', position:'relative' }}
      onDragOver={e => { if (!bulkMode) { e.preventDefault(); setDragOver(true); } }}
      onDragLeave={e => { if (!e.currentTarget.contains(e.relatedTarget)) setDragOver(false); }}
      onDrop={e => { e.preventDefault(); setDragOver(false); const id = parseInt(e.dataTransfer.getData('candidateId')); if (id) onDrop(id, stage.id); }}
    >
      <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', padding:'8px 10px', marginBottom:'8px', borderRadius:'8px', background: hex2rgba(stage.color, 0.12) }}>
        <div style={{ display:'flex', alignItems:'center', gap:'7px', minWidth:0 }}>
          ...
          <span style={{ fontSize:'0.68rem', fontFamily:'DM Mono', flexShrink:0, background: hex2rgba(stage.color, 0.25), color:stage.color, padding:'1px 7px', borderRadius:'10px' }}>
        ...
        <div style={{ display:'flex', gap:'1px', alignItems:'center', position:'relative', flexShrink:0 }}>
          ...
          <button onClick={() => setShowEditor(v => !v)} style={{ background:'none',border:'none',cursor:'pointer',color:'var(--muted)',fontSize:'1rem',padding:'2px 4px',borderRadius:'4px' }}>⋯</button>
          {showEditor && (
            <StageEditor stage={stage} onSave={s => { onEdit(s); setShowEditor(false); }} onDelete={onDelete} onClose={() => setShowEditor(false)} />
          )}
        </div>
      </div>
      ...
      <div style={{ flex:1, display:'flex', flexDirection:'column', gap:'8px', overflowY:'auto', minHeight:'60px', maxHeight:'calc(100vh - 260px)', padding:'0 2px 4px' }}>
        {candidates.map(c => (
          <CandidateCard key={c.id} candidate={c} onOpen={onOpen} bulkMode={bulkMode} selected={selectedIds.includes(c.id)} onToggle={onToggle} />
        ))}
        {candidates.length === 0 && (
  ...
  return (
    <div style={{ display:'flex', gap:'8px', flexWrap:'wrap', alignItems:'center', padding:'10px 0', borderBottom:'1px solid var(--border)', marginBottom:'12px' }}>
      <div style={{ display:'flex', alignItems:'center', gap:'6px', background:'var(--bg)', border:'1px solid var(--border)', borderRadius:'7px', padding:'6px 10px', flex: isMobile ? '1 1 100%' : '1 1 180px', maxWidth:'260px' }}>
        ...
        <input value={filters.search} onChange={e => onChange('search', e.target.value)} placeholder="Пошук кандидата..." style={{ border:'none', background:'transparent', outline:'none', fontSize:'0.8rem', fontFamily:'DM Sans', width:'100%' }} />
        {filters.search && <button onClick={() => onChange('search','')} style={{ background:'none',border:'none',cursor:'pointer',color:'var(--muted)',fontSize:'0.8rem',padding:0 }}>✕</button>}
      </div>
...
function Kanban({ searchQuery = '' }) {
  ...
  const handleCopyOrgTemplate = async () => {
    ...
    try { const res = await axios.post(`/api/vacancies/${currentVacancyId}/copy_org_stages/`); setStages(res.data); setIsOrgTemplate(false); }
  ...
  const handleResetToOrg = async () => {
    ...
    try { const res = await axios.post('/api/vacancy-stages/reset_to_org/', { vacancy_id: currentVacancyId }); setStages(res.data); setIsOrgTemplate(true); }
    catch (err) { console.error(err); }
  };
  ...
  const getCandidatesForStage = (stageId) => candidates.filter(c => (c.stage_id ?? c.stage) === stageId);
    ...
    <div style={{ display:'flex', flexDirection:'column', height:'100%', gap:0 }}>
      ...
      <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', gap:'10px', flexWrap:'wrap', marginBottom:'8px', flexShrink:0 }}>
        ...
        <div style={{ display:'flex', gap:'8px', alignItems:'center', flexWrap:'wrap' }}>
          {/* Bulk toggle */}
          <button onClick={() => { setBulkMode(v => !v); setSelectedIds([]); }} style={{
            padding:'6px 12px', borderRadius:'7px', fontSize:'0.78rem',
            border:`1px solid ${bulkMode ? 'var(--accent)' : 'var(--border)'}`,
            background: bulkMode ? 'rgba(159,18,57,0.08)' : 'var(--surface)',
            color: bulkMode ? 'var(--accent)' : 'var(--muted)',
            cursor:'pointer', fontFamily:'DM Mono', transition:'all 0.15s',
          }}>
          ...
          <div style={{ position:'relative' }}>
            <button onClick={() => setShowAddStage(v => !v)} style={{ padding:'6px 14px', borderRadius:'7px', border:'none', background:'var(--accent)', color:'#fff', cursor:'pointer', fontSize:'0.78rem', fontWeight:600 }}>+ Етап</button>
            {showAddStage && (
              <StageEditor stage={null} onSave={s => { handleSaveStage(s); setShowAddStage(false); }} onDelete={null} onClose={() => setShowAddStage(false)} />
            )}
          </div>
        </div>
      </div>
      ...
      <BulkActionBar
        ...
        onDone={() => { loadCandidates(); setSelectedIds([]); }}
      />
      ...
      {selectedCandidateId && (
        <CandidateCardModal
          ...
          onStatusChange={(id, stageId) => setCandidates(prev => prev.map(c => c.id === id ? { ...c, stage: stageId, stage_id: stageId } : c))}
          onDelete={(id) => { setCandidates(prev => prev.filter(c => c.id !== id)); setSelectedCandidateId(null); }}
        />
      )}
    </div>
  );
