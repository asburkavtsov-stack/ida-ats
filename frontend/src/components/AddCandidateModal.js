...
function AddCandidateModal({ onClose, onAdded }) {
  ...
  const handleChange = e => {
    ...
    // Очищаємо попередження про дублікат при зміні email/phone
    if (e.target.name === 'email' || e.target.name === 'phone') {
      setDuplicateWarning(null);
    }
  };

  // Перевірка на дублікат перед відправкою
  const checkDuplicate = async () => {
    const email = form.email.trim();
    const phone = form.phone.trim();

    if (!email && !phone) {
      return null; // Немає чого перевіряти
    }
  ...
  // Перевірка дубліката при втраті фокусу з email або phone
  const handleBlur = async (e) => {
    ...
    if (field === 'email' || field === 'phone') {
      ...
      // Перевіряємо тільки якщо хоч щось заповнено
      if (email || phone) {
  ...
  const handleSubmit = async () => {
    ...
    // Фінальна перевірка на дублікат перед відправкою
    const duplicateData = await checkDuplicate();
    if (duplicateData) {
      setDuplicateWarning(duplicateData);
